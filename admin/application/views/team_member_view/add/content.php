<div class="content">
    <div class="panel-header bg-primary-gradient">
        <div class="page-inner">
            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                <div>
                    <h2 class="text-white pb-4 fw-bold"><?php echo $pageTitle; ?></h2>
                </div>
            </div>
        </div>
    </div>
    <div class="page-inner mt--5">
        <form action="<?php echo base_url("team_member/save"); ?>" method="post" enctype="multipart/form-data">
            <div class="row row-card-no-pd">
                <div class="col-md-12">
                    <div class="form-group">
                        <label>Full Name</label>
                        <input type="text" 
                            class="form-control" 
                            name="title"
                            placeholder="Enter Full Name">
                            <?php if(isset($form_error)){ ?>
                                <small class="form-text text-muted text-danger"><?php echo form_error("title"); ?></small>
                            <?php } ?>
                    </div>
                    <div class="form-group">
                        <label>Designation</label>
                        <input type="text" 
                            class="form-control" 
                            name="designation"
                            placeholder="Enter Designation"
                            value="<?php echo isset($form_error) ? set_value("designation") : ""; ?>">
                    </div>
                    <div class="form-group">
                        <label>Phone Number</label>
                        <input type="text" 
                            class="form-control" 
                            name="phone_number"
                            placeholder="Enter Phone Number"
                            value="<?php echo isset($form_error) ? set_value("phone_number") : ""; ?>">
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                <label>Email Address</label>
                                <input type="email" 
                                    class="form-control" 
                                    name="email"
                                    placeholder="Enter Email Address"
                                    value="<?php echo isset($form_error) ? set_value("email") : ""; ?>">
                            </div>
                            <div class="col">
                                <label>Website</label>
                                <input type="url" 
                                    class="form-control" 
                                    name="website"
                                    placeholder="Enter Website"
                                    value="<?php echo isset($form_error) ? set_value("website") : ""; ?>">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                <label>Social Media Account Icon 1</label>
                                <input type="text" 
                                    class="form-control" 
                                    name="account_icon1"
                                    placeholder="for example: fab fa-instagram"
                                    value="<?php echo isset($form_error) ? set_value("account_icon1") : ""; ?>">
                            </div>
                            <div class="col">
                                <label>Social Media Account Link 1</label>
                                <input type="url" 
                                    class="form-control" 
                                    name="account_link1"
                                    placeholder="Enter Social Media Account Link 1"
                                    value="<?php echo isset($form_error) ? set_value("account_link1") : ""; ?>">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col">
                                <label>Social Media Account Icon 2</label>
                                <input type="text" 
                                    class="form-control" 
                                    name="account_icon2"
                                    placeholder="for example: fab fa-facebook"
                                    value="<?php echo isset($form_error) ? set_value("account_icon2") : ""; ?>">
                            </div>
                            <div class="col">
                                <label>Social Media Account Link 2</label>
                                <input type="url" 
                                    class="form-control" 
                                    name="account_link2"
                                    placeholder="Enter Social Media Account Link 2"
                                    value="<?php echo isset($form_error) ? set_value("account_link2") : ""; ?>">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <label>About</label>
                        <textarea name="description" id="editor">
                            <?php echo isset($form_error) ? set_value("description") : ""; ?>                        
                        </textarea>
                    </div>
                    <div class="form-group">
                        <label>Select Image</label>
                        <input type="file" name="img_url" class="form-control">                   
                    </div>
         
                    <a href="<?php echo base_url("team_member"); ?>" class="btn btn-outline-danger mt-3">Back to Team Members</a>
                    <button class="btn btn-indigo mt-3">Submit</button>
                </div>
            </div>
        </form>
    </div>
</div>

